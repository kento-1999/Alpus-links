...
export default function PublisherOrderDetailPage() {
  ...
  // Handle submitting approval with post URL
  const handleSubmitApproval = async () => {
    if (!orderId) return
    
    // Validate post URL for guest post orders
    if ((order?.type === 'guestPost' || order?.type === 'writingGuestPost') && !postUrl.trim()) {
      toast.error('Please enter the published post URL')
      return
    }
    ...
    if (postUrl.trim()) {
      try {
        ...
        toast.error('Please enter a valid URL (e.g., https://example.com/post)')
        return
      }
    }
    ...
    try {
      ...
      const note = postUrl.trim() 
        ? `Order sent for advertiser approval. Published URL: ${postUrl.trim()}`
        : 'Order sent for advertiser approval'
          ...
          {showApprovalModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
                ...
                {(order?.type === 'guestPost' || order?.type === 'writingGuestPost') && (
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      Enter the URL where the post has been published
                    </p>
                  </div>
                )}
                ...
                <div className="flex space-x-3">
                  ...
                  >
                    {submittingApproval ? 'Sending...' : 'Send for Approval'}
                  </button>
                </div>
              </div>
            </div>
